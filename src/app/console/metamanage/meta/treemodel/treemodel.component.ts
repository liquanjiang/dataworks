...
export class TreemodelComponent implements OnInit {
    ...
    newFolder() {
        ...
        this.apiService.updateTree(newFolder).then(res => {
            if (res.code === 200) {
    ...
    deleteFolder(treeNode) {
        this.apiService.deleteFolder(treeNode.pk).then((res) => {
            if (res.code === 200) {
                Util.showMessage(res.msg, 'success');
                let parentNode = this.zTreeObj.getNodeByParam('pk', treeNode.pkParent);
                ...
                this.querydirPK.emit(parentNode.pk);
            } else {
                Util.showMessage(res.msg, 'error');
            }
        });
    ...
    initAllzTree() {
        const that = this;
        // 用于在点击父目录时进行展开和折叠
        that.apiService.getdirTree().then(res => {
            that.dataztreeNodes = res;
            if (that.dataztreeNodes.length >= 0) {
                this.zTreeObj = $.fn.zTree.getZTreeObj('dataTreeID');
                setTimeout(() => {
                    if (that.dataztreeNodes.length > 0) {
                        const currentNode = this.zTreeObj.getNodeByParam('pk', that.dataztreeNodes[0].pk);
    ...
    onHoverDomClick() {
        ...
        return (treeObj, ev, treeId, treeNode) => {
            ...
            if (ev.target.id === 'delete') {
                ...
                const pk = treeNode.pk;
                that.apiService.ifConfirmDel(pk).then((res) => {
                    if (res.code === 200) {
                        if (res.msg === '3') {
                            Util.showConfirm('该目录下有任务，确定要删除吗?', () => {
